  ...
  fn compile(&self) -> PyResult<Program> {
    match self {
      ...
      Mod::Expression { body: _ } | Mod::Interactive { body: _ } => {
        not_supported("Only modules can be compiled.")
      }
  ...
  fn compile(&self) -> PyResult<Exp> {
    match self {
      ...
      Constant::Str(string) => {
        ...
        let string = string.chars().rfold(nil, |tail, ch| Exp::Ctr {
          name: "String.cons".into(),
          args: vec![Exp::Unsigned { numb: ch as u64 }, tail],
        });
        Ok(string)
      }
  ...
  fn compile(&self) -> PyResult<Exp> {
    match self {
      ...
      ExprKind::UnaryOp { op, operand } => {
        ...
        match op {
          ...
          Unaryop::USub => {
            let term = Exp::BinOp {
              op: Oper::Sub,
              left: Box::new(Exp::Unsigned { numb: 0 }),
              right: Box::new(value),
            };
            Ok(term)
          }
      ...
      ExprKind::Compare { left, ops, comparators } => {
        ...
        let term = comparisons.fold(base, |acc, val| Exp::BinOp {
          op: Oper::And,
          left: Box::new(acc),
          right: Box::new(val),
        });
      ...
      ExprKind::List { elts, ctx } => {
        ...
        let list = exprs
          .into_iter()
          .rfold(nil, |list, exp| Exp::Ctr { name: "List.cons".into(), args: vec![exp, list] });
        Ok(list)
      }
