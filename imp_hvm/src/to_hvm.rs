...
pub fn compile_rule(fn_name: &fun::Id, mut rule: fun::Rule) -> Result<hvm::File, ()> {
...
pub fn compile_expr(expr: &fun::Expr) -> Result<Box<hvm::Term>, ()> {
    ...
    let term = match expr {
        ...
        Expr::Let { name, expr, body } => Term::Let { name: name.clone(), expr: compile_expr(expr)?, body: compile_expr(body)? },
        ...
        Expr::BinOp { op, left, right } => Term::Op2 { oper: *op, val0: compile_expr(left)?, val1: compile_expr(right)? },
        Expr::Lambda { var, body } => Term::Lam { name: var.clone(), body: compile_expr(body)? },
        Expr::MatchExpr { .. } => unreachable!("Match expressions should be hoisted before converting to HVM"),
    };
    Ok(Box::new(term))
}
...
pub fn hoist_matches(expr: &mut fun::Expr, fn_name: &fun::Id,  hoisted: &mut Vec<fun::Function>) {
    ...
    match expr {
        ...
        Expr::MatchExpr { scrutinee, cases } => {
            ...
            let rules: Vec<fun::Rule> = Vec::with_capacity(cases.len());
            for case in cases {
                let ctx = get_unbound_vars(&case.body);
                todo!();
            }
            *expr = Expr::FunCall { name: aux_name.clone(), args: vec![*scrutinee.clone()]};
        }
    }
}
